...
function print_listing_li($item_id, $title, $desc, $price, $num_bids, $end_time): void {
    if (!($end_time instanceof DateTime)) $end_time = new DateTime($end_time);
    $desc_short = strlen($desc) > 250 ? substr($desc, 0, 250) . '...' : $desc;
    $bid_label = ((int)$num_bids === 1) ? ' bid' : ' bids';
    $now = new DateTime();
    $time_remaining = $now > $end_time ? 'This auction has ended' : display_time_remaining(date_diff($now, $end_time)) . ' remaining';

...
//1. Validate bid price>0 before everything
    if ($bidPrice <= 0) { 
        return [
            "success" => false,
            "message" => "Sorry, your bid price must be higher than £0.00."
        ];
    }
    $db = get_db_connection();
...
$currentHighest = getHighestBidForAuction($auctionId);
  ...
  //(3.1) if no bids, the first bid must be £5 higher than starting price.(bid rule is not just > 0)
if ($currentHighest === null) {
    $minFirstBid = $startPrice + 5.00;  // 起拍价 + £5
    if ($bidPrice < $minFirstBid) {
        return [
            "success" => false,
            "message" => "Sorry, you are placing the first bid for this auction, it must be at least £5 higher than the starting price."
        ];
    }
}

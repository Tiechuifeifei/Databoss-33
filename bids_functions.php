...
function placeBid($buyerId, $auctionId, $bidPrice)
{
    ...
    $sqlAuction = "
        SELECT sellerId, auctionEndTime
        FROM auctions
        WHERE auctionId = ?
        LIMIT 1
    ";
    ...
    $auctionEndTime = $auctionRow["auctionEndTime"]; // datetime 字符串，例如 "2025-11-23 20:00:00"

    // Branching logic:
    // 3.1 sellers are not allowed to place a bid on his/her own auction 不允许给自己拍卖出价（buyerId 和 sellerId 不能一样）
    if ($buyerId == $sellerId) {
        return [
            "success" => false,
    ...
    // 3.2 check the auction status, cannot place a bid if aucion is finished 拍卖已结束，不能再出价
    // 用PHP的时间函数做一个简单判断：把数据库里的 datetime 转成时间stamp，再和现在的时间比较。
    if (!empty($auctionEndTime)) {
        $endTimestamp = strtotime($auctionEndTime); // 把 "2025-11-23 20:00:00" 变成一个整数时间stamp
        $nowTimestamp = time();                     // 现在的服务器时间
    ...
    // 如果 currentHighest === null：说明目前还没有任何出价，
    // 只要 bidPrice > 0 就已经通过了（上面第 1 步已经检查 > 0 了），不用再写一个 if。
    if ($currentHighest !== null) {

        // get the current highest bid price
        $cHP = (float)$currentHighest["bidPrice"]; 
        
        // if new bid <= current highest, reject it
        if ($bidPrice <= $cHP) {
            return [
                "success" => false,
                "message" => "Your bid amount must be greater than the current highest bid."
            ];
        }
    }
...
WHERE a.sellerId = ?
ORDER BY a.auctionStartTime DESC, b.bidTime DESC
";
